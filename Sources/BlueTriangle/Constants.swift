...
enum Constants {
   ...
   /* static func configEndPoint(for siteId: String) -> URL? {
    ...
    static func configEndPoint(for siteId: String) -> URL? {
        guard let url = URL(string: "http://localhost:5005/\(siteId)/config.js?siteID=\(siteId)&os=\(Constants.os)&osver=\(Device.osVersion)&app=\(Device.appVersion)&sdk=\(Device.sdkVersion)") else {
            return nil
        }
        return url
    }
